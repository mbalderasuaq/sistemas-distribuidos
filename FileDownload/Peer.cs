...
public class Peer
{
    ...
    public async Task DownloadFile(string peerIP, int peerPort, string fileName, string savePath, CancellationToken cancellationToken)
    {
        ...
        Console.WriteLine($"El archivo {fileName} se ha descargado correctamente.");
    }
    ...
    private async Task HandleClient(CancellationToken cancellationToken)
    {
        ...
        if (stream != null)
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                Console.WriteLine($"El archivo {fileName} se ha enviado correctamente.");
            }
            else
            {
                var errorMessage = Encoding.UTF8.GetBytes("El archivo solicitado no existe.");
